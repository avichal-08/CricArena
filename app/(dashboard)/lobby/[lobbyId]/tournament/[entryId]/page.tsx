...
export default async function MatchEntry({ params }: { params: { entryId: string } }) {
  ...
  const [matchEntry] = await db.select().from(matchEntries).where(eq(matchEntries.id, entryId));
  ...
  // Safely cast points breakdown to a Record
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto min-h-screen flex flex-col bg-background pb-24">
      ...
      {/* Applied the mobile height fix here */}
                  ...
                  return (
                    <div key={player.id} className="flex flex-col items-center group cursor-pointer w-[72px] sm:w-[88px]">
                      <div className="relative">
                        ...
                        {/* Individual Points Badge (Top Right) */}
                        {points !== null && (
