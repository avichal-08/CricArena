...
import { Trophy, Swords, CalendarDays, ChevronRight, Lock, Globe, ShieldCheck, Zap, Plus } from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto w-full p-5 md:p-8 space-y-8 pb-24">
      <div className="flex items-center gap-3 pb-6 border-b border-white/[0.05]">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Active Arenas</h1>
          <p className="text-[12px] text-stone-500 mt-0.5">Your ongoing matches and tournaments</p>
        </div>
      </div>
      ...
      {activeLobbies.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-white/[0.08] text-center">
          ...
          <h2 className="text-base font-bold text-stone-400 mb-2">No active arenas</h2>
          <p className="text-sm text-stone-600 max-w-xs mb-6">Create a new lobby or ask a friend to share their invite link.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeLobbies.map((lobby) => (
            <Link key={lobby.id} href={`/lobby/${lobby.id}`}>
              <div className="group flex flex-col justify-between h-full p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-orange-500/20 hover:bg-orange-500/[0.03] transition-all duration-200 active:scale-[0.98]">
